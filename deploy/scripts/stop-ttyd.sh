...
if [ -f "$PIDFILE_CLIP" ]; then
    ...
    CLIP_PID="$(cat "$PIDFILE_CLIP")"
    kill "$CLIP_PID" 2>/dev/null
    # Xvfb removes its socket asynchronously. Wait before deleting the lock
    # and allowing a replacement server to claim the same display.
    for _ in $(seq 1 40); do
        kill -0 "$CLIP_PID" 2>/dev/null || break
        sleep 0.1
    done
    rm -f \
        "$PIDFILE_CLIP" \
        "/tmp/.X11-unix/X${CLIP_DISPLAY#:}" \
        "/tmp/.X${CLIP_DISPLAY#:}-lock" \
        2>/dev/null
