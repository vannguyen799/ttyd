...
build_openssl() {
    ...
    pushd "${BUILD_DIR}/openssl-${OPENSSL_VERSION}"
        ...
        case ${BUILD_TARGET} in
            ...
            win32)
                curl -fSsL --retry 5 --retry-delay 2 --retry-all-errors \
                    https://github.com/openssl/openssl/pull/29826.patch | patch -p1
                ;;
        esac
...
build_libwebsockets() {
    ...
    curl -fSsL --retry 5 --retry-delay 2 --retry-all-errors \
        "https://github.com/warmcat/libwebsockets/archive/v${LIBWEBSOCKETS_VERSION}.tar.gz" \
        | tar xz -C "${BUILD_DIR}"
